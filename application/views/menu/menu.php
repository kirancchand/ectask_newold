...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
        ...
        <div class="box-body">
          <form class="form-horizontal" id="menuform_list" method="post" action="<?php echo site_url('menu/add_menu'); ?>"> 
                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Add New Menu</label>
                  <div class="col-sm-4">
                    <input type="text" class="form-control" id="menu" name="menu"placeholder="Add Menu">
                  </div>
                   <div class="col-sm-4">
                    <input type="text" class="form-control" id="route" name="route"placeholder="Add Route">
                  </div>
                  <div class="col-sm-2">
...
<!--                       <button type="button" class="btn btn-danger delete_btn" id="<?php echo $value["id"];?>">Delete</button> -->
                    </div></td>
                  </tr>
                  <?php
                 }
...
<script>
  $(document).ready(function() {
    $("#menuform_list").validate({
      rules: {
        menu: "required",   
        route:"required",        
        },
        messages: {
          menu: "This field conot be empty",   
          route:"This field conot be empty",
    ...
    $("#menuform").validate({
      rules: {
        ...
        messages: {
          menuname: "This field conot be empty",   
          routename:"This field cannot be empty",
    ...
    $('.update_getbtn').click(function(){
       ...
       $.ajax({
                    type: "POST",
                    url: "<?php echo site_url(); ?>/menu/get_menu",
                    dataType : "json",
                    data: {"menu_id":menu_id},
                    success: function(response){
                          if(response)
                          {
                              $('#menuname').val(response[0].menu);
                              $('#routename').val(response[0].route);
    ...
    $('.update_btn').click(function(){
       ...
       $.ajax({
                    type: "POST",
                    url: "<?php echo site_url(); ?>/menu/update_menu",
                    data:{"route":route,"menu":menu,"id":id},
                    dataType : "json",
                    success: function(response){
