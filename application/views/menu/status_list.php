  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="box">
        ...
        <div class="box-body">
         <form class="form-horizontal" id="form_status_list"method="post" action="<?php echo site_url('user/add_status');?>">
                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Add New status</label>
                  <div class="col-sm-8">
                    <input type="text" class="form-control" id="status" name="status" placeholder="Add status">
                  </div>
                  <div class="col-sm-2">
...
<form name="statustypeform" id="statustypeform">
  <!-- /.content-wrapper -->
      ...
      <div class="modal-body">
        <p>
          <div class="form-group has-feedback">
            <label>Status</label>
        <input type="text" id="statusname" name="statusname" class="form-control"  required="required">
        </div>
  ...
  $(document).ready(function() {
    $("#statustypeform").validate({
      rules: {
        ...
        messages: {
          statusname: "This field conot be empty",   
            
           
        }
        
    });
    $("#form_status_list").validate({
      rules: {
        status: "required",
        ...
        messages: {
          status: "This field conot be empty",   
    ...
    $('.update_getbtn').click(function(){
       ...
       $.ajax({
                    type: "POST",
                    url: "<?php echo site_url(); ?>/status/get_statusname",
                    dataType : "json",
                    data: {"status_id":status_id},
                    success: function(response){
                          if(response)
                          {
                              $('#statusname').val(response[0].status);
    ...
    $('.update_btn').click(function(){
         ...
         var id=$("#id").val();
         if($("#statustypeform").valid())
            {
       $.ajax({
                    type: "POST",
                    url: "<?php echo site_url(); ?>/status/update_statusname",
                    data:{"statusname":statusname,"id":id},
                    dataType : "json",
                    success: function(response){
